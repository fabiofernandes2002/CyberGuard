        ...
        <LinearGradient colors={['#D8DBE2', '#A9BCD0', '#A9BCD0']} style={Styles.container}>
            <View style={Styles.content}>
                ...
                <View style={Styles.progressBar}>
                    <Center w="100%">
                        <Box w="90%" maxW="400">
                            <Progress value={progress} size="lg" mx="4" colorScheme="cyan" bg="#D8DBE2" />
                        </Box>
                    </Center>
                </View>
                {/* Titulo da pergunta */}
                            ...
                            <View style={Styles.radioContainer}>
                                <Radio colorScheme="success" value={item.id} my={1}>
                                    <View style={[Styles.flatlistContiner, 
                                        {backgroundColor: selectedOption === item.id ? '#487281' : '#D9D9D9',
                                        borderColor: selectedOption === item.id ? '#D9D9D9' : '#487281'}]} 
                                    > 
                                        <Text style={{color: selectedOption === item.id ? '#f7f7f7' : '#1B1B1E'}}>{item.text}</Text>
                                    </View>
                                </Radio>
                            </View>
                        )}
                    ...
                    <View style={[Styles.button, { backgroundColor: currentQuestionNumber < totalQuestions ? '#00428A' : '#6E0271' }]}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                            <Text style={Styles.buttonText}>
                                {currentQuestionNumber < totalQuestions ? 'Próximo' : 'Terminar'}
                            </Text>
